...
local INTERNAL_GENERATION =
    (script_args[2] and script_args[2]:match("internal")) and true or false
local CPRE, CTEST
if COMPILER == "gcc" or COMPILER == "clang" then
    CPRE = COMPILER ..
               [[ -E -DIMGUI_DISABLE_OBSOLETE_FUNCTIONS -DIMGUI_API="" -DIMGUI_IMPL_API="" ]]
...
local function cimgui_generation(parser, name)
    ...
    local outpre, outpost = parser.structs_and_enums[1],
                            parser.structs_and_enums[2]
    ...
    -- merge it in cimgui_internal_template.cpp to cimguizmo.cpp
...
-----------------------------do it----------------------
--------------------------------------------------------
-- get ImGuizmo.h version--------------------------
local pipe, err = io.open("../imguizmo/ImGuizmo.h", "r")
if not pipe then error("could not open file:" .. err) end
-- local imguizmo_version
-- while true do
-- local line = pipe:read"*l"
-- imguizmo_version = line:match([[ImPlot%s+v(.+)]])
-- if imguizmo_version then break end
-- end
pipe:close()
-- cimgui_header = cimgui_header:gsub("XXX",imguizmo_version)
-- print("IMPLOT_VERSION",imguizmo_version)
...
local function custom_header(outtab, def)
    ...
    table.insert(outtab,
                 "CIMGUI_API " .. def.ret .. " " .. def.ov_cimguiname .. args ..
                     ";//custom generation\n")
    return true
end
local function custom_implementation(outtab, def)
    ...
    for i, arg in ipairs(def.argsT) do
        if arg.type == "ImPlotGetter" then
            ...
            ngetter = ngetter + 1
            table.insert(outtab, "    getter_funcX" ..
                             ((ngetter == 1 and "") or (ngetter == 2 and "2") or
                                 error "too much getters") .. " = " .. arg.name ..
                             ";\n")
            call_args = call_args ..
                            ((ngetter == 1 and "Wrapper,") or
                                (ngetter == 2 and "Wrapper2,"))
        else
...
-------------funtion for parsing ImGuizmo headers
local function parseImGuiHeader(header, names)
    ...
    parser.getCname = function(stname, funcname, namespace)
        if stname == funcname then return stname .. "_" .. funcname end
    ...
    parser.UDTs = {
        "ImVec2", "ImVec4", "ImColor", "ImRect", "ImPlotRect", "ImPlotPoint",
        "ImPlotLimits", "ImPlotTime"
    }
    -- parser.gentemplatetypedef = gentemplatetypedef
    parser.cimgui_inherited = dofile(
                                  [[../../cimgui/generator/output/structs_and_enums.lua]])
    -- this list will expand all templated functions
    parser.ftemplate_list = {}
    parser.ftemplate_list.T = {
        "float", "double", "ImS8", "ImU8", "ImS16", "ImU16", "ImS32", "ImU32",
        "ImS64", "ImU64"
    }
...
save_data("./output/structs_and_enums.lua",
          serializeTableF(structs_and_enums_table))
...
local function json_prepare(defs)
    ...
    for k, def in pairs(defs) do
        for k2, v in pairs(def) do
            if type(k2) == "string" then def[k2] = nil end
        end
    end
    return defs
end
...
save_data("./output/definitions.json",
          json.encode(json_prepare(parser1.defsT), json_opts))
save_data("./output/structs_and_enums.json",
          json.encode(structs_and_enums_table))
