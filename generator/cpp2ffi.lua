...
local function getRE()
    ...
    local resN = {
        "comment2_re", "comment_re", "emptyline_re", "functypedef_re",
        "functype_re", "function_re", "functionD_re", "typedef_st_re",
        "struct_re", "enum_re", "union_re", "namespace_re", "class_re",
        "typedef_re", "vardef_re"
    }
...
local function isLeaf(re)
    return
        (re ~= "typedef_st_re" and re ~= "struct_re" and re ~= "namespace_re" and
            re ~= "class_re" and re ~= "union_re")
end
M.getRE = getRE
    ...
    while true do
        ...
        for ire, re_name in ipairs(resN) do
            ...
            if i then
				...
				--]]
                --------------------
                if re_name == "comment_re" or re_name == "comment2_re" or
                    re_name == "emptyline_re" then
                    -- print("parit",item)
                    --[[
                    ...
                    if linenumdict then
                        ...
                        if not loca then
                            print("not loca",
                                  string.format("%q , %q ", itemold,
                                                itemfirstline), #itemfirstline)
...
local function name_overloadsAlgo(v)
    ...
    for i, t in ipairs(v) do
        ...
        aa[i] = {}
        local num = 1
        for typec in signature:gsub("(%(.-%))",
                                    function(x) return x:gsub(",", "\0") end)
                         :gmatch(".-([^,]+),?") do
    ...
    for l = 1, maxnum do
        ...
        for i = 1, #v do
            ...
            for j = 1, #v do
                if i ~= j and bb[i] == bb[j] then done[i] = false end
            end
        end
    end
...
local function clean_names_from_signature(self, signat)
    ...
    while true do
        ...
        if not typ or not name or name:match "%*" or M.c_types[name] or
            self.typedefs_dict[name] then
            print("clean_names_from_signature bad argument name", funcname, typ,
                  name, arg)
            typ = arg
            name = ""
        end
...
local function clean_functypedef(line)
    ...
    for tynam in argsp:gmatch("([^,]+),") do
        if tynam:match("%)") and not tynam:match("%b()") then
            ...
            while argsTa[#argsTa]:match("%)") and
                not argsTa[#argsTa]:match("%b()") do
                argsTa[#argsTa - 1] = argsTa[#argsTa - 1] .. "," ..
                                          argsTa[#argsTa]
                argsTa[#argsTa] = nil
            end
        else
...
local function parseFunction(self, stname, itt, namespace, locat)
    ...
    if #argsArr > 0 then
        ...
        for i, v in ipairs(argsArr) do
            if v.ret then -- function pointer
                ...
                caar = caar .. v.name .. ","
                signat = signat .. v.ret .. f_ .. ")" ..
                             clean_names_from_signature(self, v.signature) ..
                             ","
            else
                local siz = v.type:match("(%[%d*%])") or ""
                local typ = v.type:gsub("(%[%d*%])", "")
                asp = asp .. typ .. (v.name ~= "..." and " " .. v.name or "") ..
                          siz .. ","
    ...
    for i, ar in ipairs(argsArr) do
        if ar.default then
            ...
            local is_cstring = ar.default:sub(1, 1) == '"' and
                                   ar.default:sub(-1, -1) == '"'
            if not is_cstring then
                ar.default = ar.default:gsub("%(%(void%s*%*%)0%)", "NULL")
                if ar.default:match "%(ImU32%)" and
                    not ar.default:match "sizeof" then
    ...
    defT.argsT = argsArr
    if self.get_manuals(defT) then defT.manual = true end
    if self.get_skipped(defT) then defT.skipped = true end
    if ret then
...
local function AdjustArguments(FP)
    for fun, defs in pairs(FP.defsT) do
        ...
        if defs[1].stname ~= "" and defs[1].ret and
            not defs[1].is_static_function then
            -- print("adjusting",fun)
            for i, def in ipairs(defs) do
                local empty = def.args:match("^%(%)") -- no args
                -- local ptret = def.retref and "&" or ""
                def.args = def.args:gsub("^%(", "(" .. def.stname .. "* self" ..
                                             (empty and "" or ","))
                table.insert(def.argsT, 1,
                             {type = def.stname .. "*", name = "self"})
            end
        end
    end
end
    ...
    for numcdef, t in ipairs(FP.funcdefs) do
        ...
        if t.cimguiname then
            ...
            if isIMSTR then
                ...
                local defT2 = {}
                -- first strings
                for k, v in pairs(defT) do defT2[k] = v end
                -- then argsT table
                defT2.argsT = {}
                ...
                if doadd and not FP.NOCHAR then
                    ...
                    newcdefs[#newcdefs + 1] = {
                        ...
                        funcname = t.funcname,
                        args = defT2.args,
                        signature = defT2.signature,
                        cimguiname = defT2.cimguiname,
                        ret = defT2.ret
                    }
                end
    ...
    for numcdef, t in ipairs(FP.funcdefs) do
        if t.cimguiname then
            ...
            if isUDT then
                ...
                defT2.args = "(" .. defT.ret .. " *pOut" .. comma ..
                                 defT.args:sub(2)
                defT2.ret = "void"
        ...
        if t.cimguiname then
            ...
            if not defT[1].ret and not defT[1].constructor then -- if constructor not processed
                if defT[1].funcname:match("~") then
                    ...
                    newcdefs[#newcdefs + 1] = {
                        stname = def.stname,
                        funcname = def.ov_cimguiname,
                        args = def.args,
                        signature = def.signature,
                        cimguiname = def.cimguiname,
                        call_args = def.call_args,
                        ret = def.ret
                    }
                end
            end
    ...
    for k, defT in pairs(fdefs) do
        for i, def in ipairs(defT) do
            for j, arg in ipairs(def.argsT) do
                ...
                if arg.name == "*" or not arg.type or not arg.name or
                    c_types[arg.name] or typedefs[arg.name] then
                    print("bad argument name", arg.name, "in", def.funcname,
                          def.args)
                end
            end
        end
...
local function printItems(items)
    local function printItemsKind(items, kind)
        ...
        if someit then for i = 1, #someit do print(someit[i]) end end
    end
    local res, resN = getRE()
    for k, v in pairs(resN) do printItemsKind(items, v) end
end
-------------
function M.Parser()
    local par = {}
    ...
    function par.getCname_overload(stname, funcname, signature, namespace)
        ...
        local ov_cname = par.cname_overloads[cname] and
                             par.cname_overloads[cname][signature] -- or cname
        return ov_cname
    end
    ...
    function par:take_lines(cmd_line, names, compiler)
        ...
        if not pipe then error("could not execute COMPILER " .. err) end

        local defines = {}
        local preprocessed = {} --

        for line, loca, loca2 in M.location(pipe, names, defines, compiler,
                                            self.COMMENTS_GENERATION) do
    ...
    function par:Listing(arr, ff)
        for i, it in ipairs(arr) do
            ff(it)
            if not isLeaf(it.re_name) then par:Listing(it.childs, ff) end
        end
    end
        ...
        for i, it in ipairs(itsarr) do
            if not isLeaf(it.re_name) then
                ...
                if it.re_name == "struct_re" then
                    local typename = it.item:match(
                                         "^%s*template%s*<%s*typename%s*(%S+)%s*>")
                    local stname = it.item:match("struct%s+(%S+)")
                    it.name = stname

                    -- local templa1,templa2 = it.item:match("^%s*template%s*<%s*(%S+)%s*(%S+)%s*>")
                    local templa2 = it.item:match(
                                        "^%s*template%s*<%s*([^<>]+)%s*>")
                    if templa1 or templa2 then
                        print("template found", stname, templa1, templa2,
                              "typename", typename)
                    end
    ...
    function par:printItems() printItems(items) end
        ...
        for j, it in ipairs(itlist) do
            if (it.re_name == "vardef_re" or it.re_name == "functype_re") then -- or it.re_name == "union_re") then
                if not (it.re_name == "vardef_re" and it.item:match "static") then -- skip static variables
                    ...
                    if template then
                        ...
                        if doheader then
                            local templatetypedef =
                                self:gentemplatetypedef(ttype, template,
                    ...
                    table.insert(commtab, {
                        above = it.prevcomments,
                        sameline = it.comments
                    }) -- it.comments or "")
                end
            ...
            elseif it.re_name == "struct_re" then
                ...
                local cleanst, structname, strtab, comstab, predec =
                    self:clean_structR1(it, doheader)
                if doheader then
            ...
            elseif it.re_name ~= "functionD_re" and it.re_name ~= "function_re" then
                print(it.re_name, "not processed clean_struct in", stname,
                      it.item:sub(1, 24))
                -- M.prtable(it)
            end
        end
    ...
    function par:gen_structs_and_enums()
        ...
        local processer = function(it)
            if it.re_name == "typedef_re" or it.re_name == "functypedef_re" or
                it.re_name == "vardef_re" then
                if not it.parent or it.parent.re_name == "namespace_re" then
                    ...
                    if it.re_name == "typedef_re" then
                        ...
                        if template then
                            ...
                            local tdt = self:gentemplatetypedef(ttype, template,
                                                                te)
                            it2 = tdt .. code2
                        end
            ...
            elseif it.re_name == "enum_re" then
                if not (it.parent.re_name == "struct_re") then -- this ensures that we don't try to add enum predeclared in struct generation
                    ...
                    local enumname =
                        it.item:match "^%s*enum%s+c?l?a?s?s?%s*([^%s;{}]+)"
                    if enumname then
                        -- if it's an enum with int type changed
                        if self.structs_and_enums_table.enumtypes[enumname] then
                            local enumtype =
                                self.structs_and_enums_table.enumtypes[enumname]
                            local enumbody = ""
                            local extraenums = ""
                            for i, v in ipairs(
                                            self.structs_and_enums_table.enums[enumname]) do
                        ...
                        if it.parent then
                            if it.parent.re_name == "namespace_re" then
                                local namespace =
                                    it.parent.item:match("namespace%s+(%S+)")
                                self.embeded_enums[enumname] =
                                    namespace .. "::" .. enumname
                            end
                        end
            ...
            elseif it.re_name == "struct_re" or it.re_name == "typedef_st_re" then
                ...
                if it.parent then -- and (it.parent.re_name == "struct_re" or it.parent.re_name == "typedef_st_re" then
                    local embededst = (it.re_name == "struct_re" and
                                          it.item:match("struct%s+(%S+)")) or
                                          (it.re_name == "typedef_st_re" and
                                              it.item:match("%b{}%s*(%S+)%s*;"))
                    ...
                    if it.parent.re_name == "struct_re" then
                        ...
                        self.embeded_structs[embededst] =
                            "struct " .. parname .. embededst
                    else
                        self.embeded_structs[embededst] = parname .. embededst
                    end
                end
            elseif it.re_name == "namespace_re" or it.re_name == "union_re" or
                it.re_name == "functype_re" then
                -- nop
            elseif it.re_name == "functionD_re" or it.re_name == "function_re" then
                local stname = ""
                local namespace
                if it.parent then
                    if it.parent.re_name == "struct_re" or it.parent.re_name ==
                        "typedef_st_re" then
                ...
                if it.item:match "^%s*template%s+<" then
                    local ttype, fun =
                        it.item:match "^%s*template%s+<%s*typename%s+([^>]+)%s*>%s*(.+)$"
                    if self.ftemplate_list and self.ftemplate_list[ttype] then
                        for iT, vT in ipairs(self.ftemplate_list[ttype]) do
                            ...
                            self:parseFunction(stname, {item = funT}, namespace,
                                               it.locat)
                        end
                    end
                else
        ...
        local outtabprest, outtabst = table.concat(outtabpre, ""),
                                      table.concat(outtab, "")
    ...
    function par:parse_struct_line(line, outtab, comment)
        ...
        if line:match(functype_re) then
            ...
            table.insert(outtab,
                         {type = t1 .. t2, name = name, comment = comment})
        else
    ...
    local function enums_for_table(it, outtab, enumsordered)
        ...
        if not enumname then
            ...
            print("unamed enum", enumname,
                  it.parent and ("parent:" .. it.parent.name) or "no parent")
        end
        ...
        outtab.enum_comments[enumname] = {
            sameline = it.comments,
            above = it.prevcomments
        }
        outtab.enum_comments[enumname] =
            next(outtab.enum_comments[enumname]) and
                outtab.enum_comments[enumname] or nil
        ...
        local processer = function(it)
            if it.re_name == "typedef_re" or it.re_name == "functypedef_re" or
                ...
                if it.re_name == "typedef_re" and not it.parent then
                    local typedefdef, typedefname =
                        it.item:match "typedef(.+)%s([^%s;]+);$"
            ...
            elseif it.re_name == "struct_re" or it.re_name == "typedef_st_re" then
                local cleanst, structname, strtab, comstab =
                    self:clean_structR1(it)
                -- if not void stname or templated
                if not structname then
                    print("NO NAME", cleanst, it.item)
                end
                if structname and not self.typenames[structname] then
                    ...
                    outtab.struct_comments[structname] = {
                        sameline = it.comments,
                        above = it.prevcomments
                    }
                    outtab.struct_comments[structname] = next(
                                                             outtab.struct_comments[structname]) and
                                                             outtab.struct_comments[structname] or
                                                             nil
                    outtab.locations[structname] = it.locat
                    for j = 3, #strtab - 1 do
                        self:parse_struct_line(strtab[j],
                                               outtab.structs[structname],
                                               comstab[j])
                    end
                else
        ...
        M.table_do_sorted(self.defsT, function(k, v)
            ...
            if #v > 1 then
                ...
                for i, t in ipairs(v) do
                    ...
                    t.ov_cimguiname = self.getCname_overload(t.stname,
                                                             t.funcname,
                                                             t.signature,
                                                             t.namespace) or k ..
                                          typetoStr(post[i])
                    ...
                    if not isDuplicate then
                        table.insert(strt, string.format("%d\t%s\t%s %s", i,
                                                         t.ret, t.ov_cimguiname,
                                                         t.signature))
                    end
                end
    ...
    local function typename_gsub(cad, typename, tname)
        ...
        if n == 0 then
            str, n = cad:gsub("(const%s+)(" .. typename .. ")(%s*%*)",
                              tname .. " %1%3")
        end
        if n == 0 then
            str, n = cad:gsub("([^_%w])(" .. typename .. ")([^_%w])",
                              "%1" .. tname .. "%3")
        end
        if n == 0 then
    ...
    function par:compute_templated()
            ...
            if t.cimguiname then
                ...
                if defT.templated then
                    ...
                    for tname, tnamestr in pairs(templates) do
                        ...
                        local cimguiname =
                            t.cimguiname:gsub(defT.stname, stname)
                        local ov_cimguiname =
                            defT.ov_cimguiname:gsub(defT.stname, stname)
        ...
        for i, v in ipairs(newcdefs) do table.insert(self.funcdefs, v) end
    end
    ...
    function par:gen_template_typedef_auto(ttype, te, newte)
        ...
        for i, v in ipairs(defi) do
            ...
            if templatet then
                typ = code2t
                table.insert(precode,
                             self:gentemplatetypedef(ttypet, templatet, tet))
            end
        ...
        return table.concat(precode) .. "\ntypedef struct " .. ttype .. "_" ..
                   newte .. " {" .. table.concat(code) .. "} " .. ttype .. "_" ..
                   newte .. ";\n"
    end
    return par
end
...
local function serializeTable(name, value, saved)
    ...
    elseif type(value) == "table" then
        if saved[value] then -- value already saved?
            ...
            ---[[
            local ordered_keys = {}
            for k, v in pairs(value) do table.insert(ordered_keys, k) end
        ...
        repeat
            ...
            elseif in_location then
                ...
                if keepemptylines or line:match("%S") then -- nothing on emptyline
                    if (which_locationold ~= which_location) or
                        (loc_num_realold and loc_num_realold < loc_num_real) then
                        -- old line complete
                        -- doprint = false
                        local lineR, which_locationR, loc_num_realR = lineold,
                                                                      which_locationold,
                                                                      loc_num_realold
                        lineold, which_locationold, loc_num_realold = line,
                                                                      which_location,
                                                                      loc_num_real
                        return lineR, which_locationR, loc_num_realR
                    else
                        lineold = lineold .. line
                        which_locationold, loc_num_realold = which_location,
                                                             loc_num_real
...
local function ImGui_f_implementation(outtab, def)
    local ptret = def.retref and "&" or ""
    table.insert(outtab, "CIMGUI_API" .. " " .. def.ret .. " " ..
                     def.ov_cimguiname .. def.args .. "\n")
    table.insert(outtab, "{\n")
    local namespace = def.namespace and def.namespace .. "::" or ""
    namespace = def.is_static_function and namespace .. def.stname .. "::" or
                    namespace
    if def.isvararg then
        ...
        if def.ret ~= "void" then
            table.insert(outtab,
                         "    " .. def.ret .. " ret = " .. namespace ..
                             def.funcname .. "V" .. call_args .. ";\n")
        else
            table.insert(outtab, "    " .. namespace .. def.funcname .. "V" ..
                             call_args .. ";\n")
        end
    ...
    elseif def.nonUDT then
        if def.nonUDT == 1 then
            table.insert(outtab, "    *pOut = " .. namespace .. def.funcname ..
                             def.call_args .. ";\n")
        end
    else -- standard ImGui
        table.insert(outtab,
                     "    return " .. ptret .. namespace .. def.funcname ..
                         def.call_args .. ";\n")
    end
    table.insert(outtab, "}\n")
end
local function struct_f_implementation(outtab, def)
    ...
    if def.isvararg then
        ...
        if def.ret ~= "void" then
            table.insert(outtab,
                         "    " .. def.ret .. " ret = self->" .. def.funcname ..
                             "V" .. call_args .. ";\n")
        else
            table.insert(outtab, "    self->" .. def.funcname .. "V" ..
                             call_args .. ";\n")
        end
        table.insert(outtab, "    va_end(args);\n")
        if def.ret ~= "void" then
    ...
    elseif def.nonUDT then
        if def.nonUDT == 1 then
            table.insert(outtab, "    *pOut = self->" .. def.funcname ..
                             def.call_args .. ";\n")
        end
    else -- standard struct
        table.insert(outtab,
                     "    return " .. ptret .. "self->" .. def.funcname ..
                         def.call_args .. ";\n")
    end
    table.insert(outtab, "}\n")
end
local function func_implementation(FP)
    ...
    for _, t in ipairs(FP.funcdefs) do
        repeat -- continue simulation
            ...
            if not custom and not manual and not def.templated and
                not FP.get_skipped(def) then
                if def.constructor then
                    assert(def.stname ~= "", "constructor without struct")
                    local empty = def.args:match("^%(%)") -- no args
                    table.insert(outtab,
                                 "CIMGUI_API " .. def.stname .. "* " ..
                                     def.ov_cimguiname ..
                                     (empty and "(void)" or def.args) .. "\n")
                    table.insert(outtab, "{\n")
                    table.insert(outtab, "    return IM_NEW(" .. def.stname ..
                                     ")" .. def.call_args .. ";\n")
                    table.insert(outtab, "}\n")
...
local function table_do_sorted(t, f)
    ...
    table.sort(sorted)
    for ii, k in ipairs(sorted) do f(k, t[k]) end
end
    ...
    for _, t in ipairs(FP.funcdefs) do
        if t.cimguiname then
            ...
            if not custom and not manual and not def.templated and
                ...
                elseif def.destructor then
                    table.insert(outtab,
                                 "CIMGUI_API void " .. def.ov_cimguiname ..
                                     def.args .. ";" .. addcoment .. "\n")
...
local function func_header_generate(FP)
    ...
    for i, v in ipairs(FP.funcdefs) do
        ...
        for j = i + 1, #FP.funcdefs do
            if v.cimguiname == FP.funcdefs[j].cimguiname and v.signature ==
                FP.funcdefs[j].signature then
        ...
        if not isDup then table.insert(newFuncdefs, v) end
    end
